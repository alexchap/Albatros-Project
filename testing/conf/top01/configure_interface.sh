...
already_done=`cat /etc/iproute2/rt_tables | egrep "(rta|rtb)" | wc -l`
...
echo "In order to connect to respective daemon, please use the commands :"
echo "sudo birdc -s /usr/local/var/run/bird_rta.ctl"
echo "sudo birdc -s /usr/local/var/run/bird_rtb.ctl"
echo 
echo "Check the logs of the daemon"
echo "tail -f /var/log/bird-top01-rta.log"
echo "tail -f /var/log/bird-top01-rtb.log"
