...
def clusterlist_to_str(cluster_list):
    ...
    for cluster in cluster_list.list:
        str += '%s ' % socket.inet_ntoa(struct.pack('>L',cluster))
    str = str[:-1]
    return str

def out_msg(out, mrt_h, bgp_h, bgp_m):
    ...
    for attr in bgp_m.update.attributes:
        if attr.type == bgp.ORIGIN:
            ...
            aggregator = 'AS%d %s' % \
                         (attr.aggregator.asn,
                          socket.inet_ntoa(struct.pack('>L',attr.aggregator.ip)))
        elif attr.type == bgp.ORIGINATOR_ID:
            originator_id = socket.inet_ntoa(struct.pack('>L',attr.originator_id.value))
    ...
    out('FROM: %s AS%d\n' % (socket.inet_ntoa(struct.pack('>L',bgp_h.src_ip)), bgp_h.src_as))
    out('TO: %s AS%d\n' % (socket.inet_ntoa(struct.pack('>L',bgp_h.dst_ip)), bgp_h.dst_as))
    if origin:
        out('ORIGIN: %s\n' % (origin))
    if as_path:
        out('ASPATH: %s\n' % (as_path))
        ...
        for mrt_h, bgp_h, bgp_m in dump:
            ...
            for attr in bgp_m.update.attributes:
                if attr.type == bgp.AS_PATH:
                    ...
                    aggregator = socket.inet_ntoa(struct.pack('>L',attr.aggregator.ip))
            ...
            for route in bgp_m.update.announced:
                out('%s|A|%s|%d|%s/%d|%s\n' % 
                    (time_str, socket.inet_ntoa(struct.pack('>L',bgp_h.src_ip)), bgp_h.src_as,
                     socket.inet_ntoa(struct.pack('>L',route.prefix)), route.len, attrs))

            for route in bgp_m.update.withdrawn:
                out('%s|W|%s|%d|%s/%d\n' % 
                    (time_str, socket.inet_ntoa(struct.pack('>L',bgp_h.src_ip)), bgp_h.src_as,
                     socket.inet_ntoa(struct.pack('>L',route.prefix)), route.len))
    else:
        for mrt_h, bgp_h, bgp_m in dump:
            out_msg(out, mrt_h, bgp_h, bgp_m)
